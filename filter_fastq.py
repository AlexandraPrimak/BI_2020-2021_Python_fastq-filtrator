...
def validate_read(fastq_read, min_length, gc_bounds):
    ...
    if len(gc_bounds) == 1:
        if count_GC(fastq_read) < gc_bounds[0]:
            return False
    if len(gc_bounds) == 2:
        isLess = count_GC(fastq_read) < gc_bounds[0]
        isMore = count_GC(fastq_read) > gc_bounds[1]
        if isLess or isMore:
            return False
    return True
...
while i < len(sys.argv) - 1:
    ...
    if sys.argv[i] == GC_BOUNDS_ARG:
        ...
        while sys.argv[i].isnumeric():
            ...
            i += 1
        if len(gc_bounds) > 2:
            errorText = GC_BOUNDS_ARG + ' must contain only up to two values'
...
total_reads_number_str = str(len(lines) // 4)
total_number_text = '\n ----->> total number of processed reads in '

print(total_number_text + input_file + ': ' + total_reads_number_str)
print('\n ----->> Filtering: Finish')
